...
def build_curriculum(step_1_source, step_1_weights, opt, device):

    # 1) First get model output over all unlabeled images from the target domain and compute their energy scores
    weights = step_1_weights  # data_dict['step_2']['weights']
    ...
    # Dataloader
    dataset = LoadImages(step_1_source, img_size=imgsz)
    ...
    for path, img, im0s, vid_cap in tqdm(dataset):
        ...
        # Inference
        with torch.no_grad():
            pred0 = model(img, augment=opt.augment)[0]

        # Apply NMS
        pred = non_max_suppression(pred0, opt.conf_thres, opt.iou_thres, classes=opt.classes, agnostic=opt.agnostic_nms)
        ...
        energy_value = torch.mean(1.0 * torch.logsumexp(pred0[0,:,5:]/ 1.0, dim=1)) + \
                       torch.mean(1.0 * torch.logsumexp(pred0[0,:,:4]/ 1.0, dim=1))
        energies.append((energy_value, path.replace('\\', '/')))
        paths.append(path.replace('\\', '/'))

    # 2) Create partitions
    sources_paths = []
    e_list = sorted(energies)
    for idx in range(len(e_list)):
        if idx == 0:
            p = []
        if (idx > 0) and (len(p) == int(len(e_list) / opt.num_curriculum_partitions)):
            sources_paths.append(p)
            p = []
        p.append(e_list[idx][1])
    if len(p) > opt.num_curriculum_partitions:
        sources_paths.append(p)

    # 3) Save new sub-datasets (it's technically not needed to re-save, but it makes the code cleaner & easier to use)
    sources = []
    for i, p in enumerate(sources_paths):
        dirname = os.path.basename(step_1_source) + "_partition_{}".format(i)
        dirpath = os.path.join(step_1_source, os.pardir, dirname)
        os.makedirs(dirpath, exist_ok=True)
        sources.append(dirpath)
        for one_file in p:
            shutil.copy(one_file, dirpath)
...
def adapt(data_cfg, opt):
    ...
    for iteration in range(opt.num_curriculum_partitions):
        # step 1: generate pseudo-labels from the baseline model
        ...
        step_1_has_extra = data_cfg['step_1'].get('extra_source') and data_cfg['step_1'].get('extra_labels_dir') and data_cfg['step_1'].get('extra_predictions_dir')

        if iteration == 0:
            if opt.num_curriculum_partitions > 1:
                sources = build_curriculum(step_1_source, step_1_weights, opt, device)
            else:
                sources = [step_1_source]

        step_1_source = sources[iteration]
        ...
        opt.epochs = ckpt['epoch'] + opt.epochs + 1 - int(iteration > 0) * ckpt['epoch']# add 1 because the start ckpt is ckpt['epochs'] + 1
    ...
    if not opt.teacher_adapt:
        assert step_3_weights == os.path.join(step_2_result_dir, "weights/best.pt").replace('\\', '/')
        opt.conf_thres = 0.4
        opt.iou_thres = 0.65
        opt.augment = False
        step_3_source = data_cfg['step_3']['source']
        step_3_has_extra = data_cfg['step_3'].get('extra_source') and data_cfg['step_3'].get('extra_labels_dir') and \
                           data_cfg['step_3'].get('extra_predictions_dir')

        print("\n============== START STEP: Refining pseudolabels ==============")
        with torch.no_grad():
            labeler.generate_labels(step_3_source, step_3_weights, opt, step_3_labels_dir, step_3_predictions_dir, opt.label_img_size)
            if step_3_has_extra:
    ...
    # Get best weights
    final_weights = os.path.join(step_4_result_dir, "weights/best.pt").replace('\\', '/')
    # Test
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='data/cross_domain/comic-adapt.yml', help='data.yaml path')
    parser.add_argument('--ckpt-run-id', type=str, default='pascal-source-5s', help='*.checkpoint run id; this defines initial checkpoint')
    parser.add_argument('--pseudo-run-id', type=str, default='pascal-source-5x', help='run id for pseudo-labels')
    ...
    parser.add_argument('--teacher-adapt', type=bool, default=True, help='set to true when proposed with teacher distillation')
    ...
    parser.add_argument('--num-curriculum-partitions', type=int, default=1, help='Use 1 for faster processing')
    ...
    parser.add_argument('--split', type=str, default='train', help='defines the split of the dataset')
    ...
    if data_dict['step_1'].get('extra_labels_dir'):
        step_1_extra_labels_dir = data_dict['step_1']['extra_labels_dir']
    if data_dict['step_1'].get('extra_predictions_dir'):
    ...
    if not opt.teacher_adapt:
        ...
        if data_dict['step_3'].get('extra_labels_dir'):
            step_3_extra_labels_dir = data_dict['step_3']['extra_labels_dir']
        if data_dict['step_3'].get('extra_predictions_dir'):
