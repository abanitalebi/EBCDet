...
class Model(nn.Module):
    ...
    def _initialize_biases(self, cf=None):  # initialize biases into Detect(), cf is class frequency
        ...
        for f, s in zip(m.f, m.stride):  #  from
            ...
            temp1 = math.log(8 / (640 / s) ** 2)  # obj (8 objects per 640 image)
            temp2 = math.log(0.6 / (m.nc - 0.99)) if cf is None else torch.log(cf / cf.sum())  # cls
            b = torch.cat([b[:, :4], b[:, 4].reshape([-1, 1])+temp1, b[:, 5:]+temp2], dim=1)
